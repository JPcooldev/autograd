...
Conv, pool, dropout, loss, `embedding`, and `cat` are `ops::` free functions only — see [ops.md](../ops.md).
...
| `Tensor(shape, ptr, n_elements, requires_grad)` | Copies `n_elements` elements from a raw pointer. Throws if `n_elements` does not match the shape, or if `ptr` is null when `n_elements > 0`. |
| copy / move                                     | Default. Copies or moves the metadata and `shared_ptr` handles (same buffer, handle semantics).                                              |
| `from_operation_result(shape, storage, requires_grad, grad_fn)`     | Math-op factory. Moves `storage` into a new packed owner with a gradient function.                                                                                             |
...
| `arange(start, end, step=1, requires_grad=true)` | 1-D `[start, end)` with spacing `step`. Throws if `step == 0`. Empty if the range does not progress toward `end`. |
...
| Method                                                           | Description                                                                                                                                             |
| ---------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `randint(shape, low, high, seed=nullopt)`                        | Uniform integers in `[low, high)`. **Always** `requires_grad=false` (no `requires_grad` argument). Throws if `low >= high`. |
| `randint_like(other, low, high, seed=nullopt)`                   | `randint` with `other.shape()`.                                                                                             |
| `randn(shape, requires_grad=true, seed=nullopt)`                 | i.i.d. samples from `N(0, 1)`.                                                                                              |
...
| `uniform(shape, low, high, requires_grad=true, seed=nullopt)`    | i.i.d. `U[low, high)`. Throws if `low >= high`.                                                                             |
...
`randn` / `random_gaussian` / `uniform` / Xavier / Kaiming are meant for floating `T`. Optional `seed` is `std::optional<uint64_t>` (`nullopt` → `std::random_device`). Kaiming `gain` is `sqrt(2 / (1 + negative_slope²))`. `fan_mode` must be `"fan_in"` or `"fan_out"`.

```cpp
auto w = tensor::Tensor<float>::xavier_uniform({4, 8}, 1.0, true, /*seed=*/1);
auto n = tensor::Tensor<float>::randn({2, 2}, false, 42);
auto u = tensor::Tensor<float>::uniform({3}, -0.5f, 0.5f, true, 7);
auto i = tensor::Tensor<int32_t>::randint({3}, 0, 10, /*seed=*/7);
auto t = tensor::Tensor<float>::arange(0, 5);   // {0, 1, 2, 3, 4}
```
...
| `to_vector()`     | Packed row-major **logical** values (`numel()` long). Gathers views through strides.                       |
| `copy_(src)`      | Writes `src`'s logical values into this tensor. Shape must match. Identity (`data_ptr_`, `requires_grad`, `grad_fn`) is kept. |
| `operator[]`      | Stride-aware chained indexing. Uses `offset_ + idx * strides_[0]` and returns a `TensorAccessor`.          |


Dense kernels that loop `data()[i]` for `i in 0..numel()` assume packed layout **and** `offset() == 0`. Library math ops pack first; use `operator[]` to inspect views.
...
t.to_vector();                       // {1, 4, 2, 5, 3, 6}

tensor::Tensor<float> y({3, 2}, 0.f, false);
y.copy_(t);                          // y is packed {1, 4, 2, 5, 3, 6}
```
...
Most of these are **zero-copy views** (`from_view`: new metadata, same `data_ptr_`). `view` requires a contiguous source. `reshape` / `flatten` pack first if needed. Negative dims go through `normalize_dimension`.
...
| `reshape(shape)`           | Reinterprets as `shape`. Packs first if not contiguous; `numel` must match.                                    |
| `view(shape)`              | Zero-copy only; throws if not contiguous.                                                                         |
| `flatten(start=0, end=-1)` | Merges axes `[start, end]` (inclusive) into one. Packs first if not contiguous.                                   |
...
| `narrow(dim, start, length)` | View of a slice along `dim`. Offset moves by `start * stride[dim]`. Throws on a scalar or out-of-range window. |
...
Kernels loop `data()[i]` densely after packing the inputs (`contiguous()` at the kernel door). That keeps a unit-stride loop the compiler can vectorize, instead of a strided wrapper around the buffer or a gather in every kernel. See [mechanism.md](mechanism.md#why-dense-kernels-pack-at-the-door).
...
| `log()`                        | Element-wise natural log. Throws `std::runtime_error` if any element is `≤ 0`.   |
...
Allocate a new packed result. Global reductions return shape `{}`. `sum(dim)` / `mean(dim)` / `max(dim)` / `min(dim)` drop that axis. Empty tensors throw on global `sum` / `mean` / `max` / `min`. Empty `dim` throws on `mean(dim)` / `max(dim)` / `min(dim)` / `softmax(dim)`.
...
| `max(dim)`     | Maximum along `dim`; that axis is removed. Ties keep the first index.                               |
| `min()`        | Minimum element → scalar. Ties keep the first index.                                                |
| `min(dim)`     | Minimum along `dim`; that axis is removed. Ties keep the first index.                               |
...
| `matmul(other)` | Last-two-dim product `[..., M, K] @ [..., K, N] → [broadcast(...), M, N]`. Leading dims broadcast. **Reads through strides**, so transposed inputs are correct without `contiguous()`. |
...
tensor::Tensor<float> X({4, 2, 3}, data, true);
auto E = X.matmul(B);              // {4, 2, 2}  — B broadcasts over the batch
```
